...
/// @dev Policy, agent registry and recipient allowlist are consolidated here (the PRD allows
///      folding PolicyManager / AgentRegistry into Treasury for the MVP).
///
...
contract Treasury is ITreasury, Ownable, ReentrancyGuard {
    ...
    modifier onlyAgent() {
        AgentInfo memory a = agents[msg.sender];
    ...
    function addRecipient(address recipient, bytes32 metadataHash) external onlyOwner {
        ...
        recipientMetadata[recipient] = metadataHash;
        ...
        returns (uint256 requestId)
    {
        ...
        _revertIfFailed(c);
        require(amount <= policy.approvalThreshold, "REQUIRES_HUMAN_APPROVAL");
    ...
    function _checks(
        ...
        AgentInfo memory a = agents[agent];
        c.notPaused = !paused;
        c.policyLive = policy.active && (policy.expiresAt == 0 || block.timestamp <= policy.expiresAt);
        c.agentAuthorized = a.active;
        c.agentNotExpired = a.expiresAt == 0 || block.timestamp <= a.expiresAt;
        c.recipientApproved = isRecipient[recipient];
        c.amountPositive = amount > 0;
        c.withinTxLimit = amount <= policy.maxTransactionAmount;
        c.invoiceFresh = !checkInvoice || (invoiceId != bytes32(0) && !invoiceClaimed[invoiceId]);
        c.withinDailyLimit = !checkVelocity || spentToday() + amount <= policy.dailyLimit;
        c.withinMonthlyLimit = !checkVelocity || spentThisMonth() + amount <= policy.monthlyLimit;
    ...
    function _authorizeAgent(address agent, uint64 expiresAt) internal {
        ...
        agents[agent] = AgentInfo(true, expiresAt);
